/*
**  OpenGL ES debugging support functions
...
// Mirrors gl_debug.cpp. gl_debug_level/gl_debug_breakpoint are process-wide
// cvars already declared in gl_debug.cpp - they are NOT redeclared here to
// avoid duplicate cvar registration, since both backends compile into the
// same binary.
...
//-----------------------------------------------------------------------------
//
// Updates OpenGL ES debugging state
//
//-----------------------------------------------------------------------------
...
void FGLDebug::PrintMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message)
{
	...
	if (messagesPrinted == maxMessages)
	{
		Printf("Max OpenGL ES debug messages reached. Suppressing further output.\n");
	}
